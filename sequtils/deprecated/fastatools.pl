...
while (my $line = <>) {
        ...
        $total_gc += $sequence =~ tr/gc/gc/;
        $total_at += $sequence =~ tr/at/at/;
        $total_n  += $sequence =~ tr/n/n/;
        $total_GC += $sequence =~ tr/GC/GC/;
        $total_AT += $sequence =~ tr/AT/AT/;
        $total_N  += $sequence =~ tr/N/N/;
...
my $both_GC = $total_gc + $total_GC;
my $both_AT = $total_at + $total_AT;
my $both_N = $total_n + $total_N;

my $average_length = $total_length / scalar(@seq_lengths);
my $gc_content = 100 * $both_GC / ($both_GC + $both_AT); 
my $at_content = 100 * $both_AT / ($both_GC + $both_AT); 
my $n_content  = 100 * $both_N  / $total_length;
my $lc_content = 100 * ($total_at + $total_gc) / ($both_GC + $both_AT);
...
if ($gc) { print join("\t", 'seq_id', 'perc_gc'), "\n"; }
if ($len){ print join("\t", 'seq_id', 'length' ), "\n"; }
if ($seqtab){ print join("\t", 'seq_id', 'seq' ), "\n"; }
while (my $line = <>) {
    chomp $line;
    my ($header, @seq) = split /\n/, $line;
    my $sequence = join '', @seq;
